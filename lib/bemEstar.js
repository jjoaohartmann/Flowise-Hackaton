 ...
 * @param {string} uid
 * @param {{ emoji: string, label: string, note?: string, intensity: number }} data
 */
export async function saveEmotion(uid, { emoji, label, note = "", intensity = 5 }) {
  await addDoc(collection(db, "emocoes"), {
    userId:    uid,          // chave de privacidade
    emoji,
    label,
    note,
    intensity,               // intensidade de 1 a 10
