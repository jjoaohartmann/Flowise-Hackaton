...
/** Retorna o nível do mascote baseado no streak (0–7) */
export function getMascotLevel(streakCount) {
  if (streakCount >= 50) return 7; // Mítico
  if (streakCount >= 30) return 6; // Lendário
  if (streakCount >= 21) return 5; // Veterano
  if (streakCount >= 14) return 4; // Sábio
  if (streakCount >= 7)  return 3; // Esperto
  if (streakCount >= 3)  return 2; // Gordinho
  if (streakCount >= 1)  return 1; // Caçula
  return 0;                        // Ovo
}
